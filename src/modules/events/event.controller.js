...
export const list = asyncHandler(async (req, res) => {
  ...
  res.json(await service.listEvents(req.user.id, { page, limit }));
});

export const getOne = asyncHandler(async (req, res) => {
  res.json(await service.getEvent(req.user.id, req.params.id));
});
...
export const nearbyServices = asyncHandler(async (req, res) => {
  const event = await service.getEvent(req.user.id, req.params.id);
